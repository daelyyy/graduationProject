...
def ContextualSBM(n, d, Lambda, p, mu, train_percent):
...
class dataset_ContextualSBM(InMemoryDataset):
        ...
        for name in self.raw_file_names:
            ...
            if not osp.isfile(p2f):
                ...
                _ = save_data_to_pickle(tmp_data,
                                        p2root=self.raw_dir,
                                        file_name=self.name)
                # print(f"Data saved with x shape: {tmp_data.x.shape}")

                # print(f"Generating data with n={self._n}, p={self._p}, Lambda={self._Lambda}, mu={self._mu}")
            else:
    ...
    def process(self):
        ...
        with open(p2f, 'rb') as f:
            data = pickle.load(f)
        # print(f"Raw data loaded with x shape: {data.x.shape}")  # 打印数据维度
...
if __name__ == '__main__':
    args = Config_utils.loadAndMergeConfig('data','../config/cSBM_Config.yml')
